...
description: Angular component structure and conventions for Zeffyr Music — OnPush, signal inputs/outputs/computed, inject(), styleUrl, the CSS entry-animation restart trick (NG0956-safe), and SEO for routed components. Use when creating or editing *.component.ts files.
---
...
- **OnPush is the Angular 22 zoneless default** — express it by **omitting** `changeDetection`. Only add `changeDetection: ChangeDetectionStrategy.Eager` when a component genuinely needs eager checking
- Use `styleUrl` (singular), not `styleUrls`
- Standalone by default in Angular 22 — no need for `standalone: true`
